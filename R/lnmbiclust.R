...
#'
#' @export
lnmbiclust <- function(W_count, range_G, range_Q, model, criteria,iter,permutation,X) {
  allmodels=c(
    "UUU",
    "UUG",
    "UUD",
  ...
  if(length(range_G)==1&length(range_Q)==1&length(model)==1){
    ...
    initial_guess <-
      try(initial_variational_gaussian(W_count, b_G, b_Q, b_cov,X))
    ...
    res <-
      try(Mico_bi_jensens(W_count, b_G, b_Q, pi_g, mu_g, sig_g, V, m, B_K, T_K, D_K, b_cov,
                          iter, const,beta_g,X))
    return(res)

  }
  else{
  all <- model_selection(W_count, range_G, range_Q, model,permutation,iter, const,X)
  ...
  beta_g<-initial_guess$new_beta_g
  res <-
    try(Mico_bi_jensens(W_count, b_G, b_Q, pi_g, mu_g, sig_g, V, m, B_K, T_K, D_K, b_cov,iter,
                        const,beta_g,X))
