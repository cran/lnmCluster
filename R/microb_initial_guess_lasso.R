...
initial_variational_lasso <- function(W_count, G, Q_g, cov_str,X) {
  n <- dim(W_count)[1]
  K <- dim(W_count)[2] - 1
  ...
  else if (code_B == "G" & length(unique(Q_g)) != 1)
  {
    ...
    for (g in 1:G) {
      fit <- eigen(sig_g[[g]])
      B_g[[g]] <- fit$vectors[, 1:Q_g] %*% diag(sqrt(fit$values[1:Q_g]), Q_g)
    }
    ...
    for (g in 1:G) {
      fit <- eigen(sig_g[[g]])
      s <- matrix(fit$vectors[, 1:Q_g], ncol = Q_g, nrow = K)
      l <- diag(fit$values[1:Q_g], Q_g)
